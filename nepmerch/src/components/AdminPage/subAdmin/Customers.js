...
const headCells = [
    ...
    { id: "incart", numeric: true, disablePadding: false, label: "InCart" },
    {
        id: "",
        numeric: true,
        disablePadding: false,
        ...
        if (props.selected.length > 1) alert("Please select only one product");
            ...
            props.products.map((element, ind) => {
                if (element.title == props.selected[0]) {
                    history.push(
                        `/store/${props.storeName}/admin/edit/${element._id}`
                    );
...
export default function EnhancedTable(props) {
    ...
    useEffect(() => {
        ...
        axios
            ...
            .get(
                `${USER_SERVER}/users/getUsers?storeName=${storeName}`
                
            )
            .then((response) => {
                if (response.data.success) {
                    console.log(response.data.userInfo);
                    setProducts(response.data.userInfo);
                    let temprow = [];
                    response.data.userInfo.map((element, index) => {
                        temprow.push(
                            createData(
                                element.name,
                                element.email,
